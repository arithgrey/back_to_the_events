...
/**/
function get_tematica($param){

	$id_evento =  $param["evento"];
	$query_get =  "select tematica_evento 
					from tematica t inner join evento_tematica et 
					on t.idtematica =  et.idtematica 
					where et.idevento =".$id_evento." limit 1";
...
/**/
function getTematicaByid($idevento){
		
	$query_get ="SELECT 
	t.* , et.idevento , 
	et.idtematica as idtem 	
	FROM tematica as t LEFT OUTER JOIN  evento_tematica as et 
	ON t.idtematica = et.idtematica 
	and 
	et.idevento = '".$idevento."' ORDER BY  t.tematica_evento asc";
	$result = $this->db->query($query_get);
	return $result->result_array();
}
/**/
...
/****************** ****************** ****************** ****************** ****************** */
/**/
function get_obj_permitidos_evento($param){

	$id_evento =  $param["evento"];
	$query_get = "select o.* from objetopermitido o 
	inner join evento_objetopermitido eo on o.idobjetopermitido = eo.idobjetopermitido 
	where eo.idevento = '".$id_evento."' LIMIT 30";
...
/**/
function create( $nombre , $edicion , $inicio , $termino , $id_usuario , $id_empresa){
	...
	$query_insert ="INSERT INTO evento(
		...
		if ($dbresponse) {		
			...
			/*Aquí verifico que en realidad no hay una lista de la emprea*/
			$db_response =  $this->evalua_obj_permitidos($id_evento ,  $id_empresa);
			/*Aquí me encargo de insertar objs al event*/
			$db_response =  $this->insert_lista_obj_permitidos_eventos($id_evento ,  $id_empresa );

			/*Cargamos las politicas y restriciones por default*/
			$this->inserta_contenidos_predeterminados($id_evento ,  $id_usuario );
		}
		return $id_evento;
}
/**/
...
/**/
function evalua_obj_permitidos($id_evento ,  $id_empresa){
      
      $query_get =  "SELECT idempresa FROM empresa_objetopermitido 
      ...
      if(count($num) == 0 ){        
        ...
        $info_def =  $this->get_info_def();          
        for($a=0; $a < count($info_def); $a++){ 

          $objeto =  $info_def[$a];  
          $query_insert =  "INSERT INTO objetopermitido(nombre) VALUES( '".$objeto ."');";                     
            
            $this->db->query($query_insert);
            $id_objeto =  $this->db->insert_id();                 

            $query_insert =  "INSERT INTO empresa_objetopermitido(idempresa , idobjetopermitido) 
                            VALUES('".$id_empresa."' ,  '".$id_objeto."')"; 
            $this->db->query($query_insert);
        }
...
/**/
function get_info_def(){
      $info_def = array("Bolsas pequeñas, cangureras o mochilas de un solo compartimiento.",
                  "Botellas o cilindros para hidratarse (completamente vacías).",
                  "Celulares.",
                  "Tapones de oído.",
                  ...
                  "Banderas o estandartes (no publicitarios).",                  
                  "Productos femeninos (totalmente sellados).",
                  "Medicamentos bajo receta medica (presentar la receta con una ID).",
                  "Encendedores.",
                  "Pomada para labios o Gloss.",
                  "Chicles (Sellados).",
                  "Hula Hoops.",
                  "Artículos inflables no mayor a 50 CM x 30 CM.",
                  "Totems (ligeros y con materiales totalmente suaves).",
                  "Bloqueador solar.",                  
                  "Desodorantes (en barra)",
                  "Baterías recargables.",
                  "Trajes de caracterización (que no excedan tu estatura).",
                  "Binoculares.",
                  "Chamarras.",
                  "Accesorios de leds.",
                  "Dulces (Sellados o nuevos).",
                  "Máscaras.",
                  "Confetti.",                                    
                  "Linternas.",
                  ...
                  "Bandas adhesivas para uso medico.",
                  "Penachos.");
        return  $info_def;
}
...
function insert_lista_obj_permitidos_eventos($id_evento ,  $id_empresa){
	/**/	
	$_num =  $this->evalua_lista_objetos_evento($id_evento);
	if ($_num  ==  0 ) {
...
/*Esto aplica sólo para politicas y restricciones*/
function inserta_contenidos_predeterminados($id_evento ,  $id_usuario ){	
	...
	$query_insert =  "INSERT INTO evento_contenido(idevento , idcontenido) 						
						...
						FROM plantilla_contenido  as pc 
					  	...
					  	WHERE  p.idusuario = '".$id_usuario."'  AND p.idtipo_plantilla in (3, 4 ) LIMIT 50";
