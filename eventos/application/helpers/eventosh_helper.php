...
function listobjetosp( $arreglo ,  $id_evento ){ 	
	...
	$height =  ""; 			
	if (count($arreglo) > 10 ){ $height ="style='overflow-y:scroll; overflow-x:hidden;   height: 300px;' "; }
	$btn =  "<i class='fa fa-check-square  btn-all-articulos'>
			</i>
		    ";
	$list = "<br>" .titulo_enid("Objtos permitidos en el evento ")."<br>";
	$list .=  btn_carga_todos_los_eventos(1 , "Incluir/eliminar" , "btn_incluir_objs" , $id_evento);
	$list .="<div $height >";				
	$b =1;
	...
	foreach ( $arreglo as $row) {	
		...
		$list .= n_row_12("contenedor_lista_objetos");		
