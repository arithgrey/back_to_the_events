...
/**/
function get_accesos_evento($param){	
	/**/
	$id_evento =  $param["id_evento"];	
	$query_get =  "SELECT  a.* , t.tipo FROM acceso a inner join  tipo_acceso t on a.idtipo_acceso = t.idtipo_acceso
				 WHERE idevento =  $id_evento ORDER BY fecha_registro ASC LIMIT 5 ";
...
/***/
function get_resumen_evento($param){

	$id_evento =  $param["id_evento"];	
	$query_get =  "SELECT COUNT(0)num_accesos FROM acceso WHERE idevento = $id_evento LIMIT 20";
	$result =  $this->db->query($query_get);
	$info_accesos =  $result->result_array()[0];
	$data_complete["num_accesos"] =  $info_accesos["num_accesos"];

	if ($info_accesos["num_accesos"] ==  1  ){
		$data_complete["prox_acceso"]	 = $this->get_promocion_evento($id_evento);
	}else if($info_accesos["num_accesos"] > 1  ){
		$data_complete["prox_acceso"]	 = $this->get_promocion_proximo_evento($id_evento);
	}
	return $data_complete;

}
/**/
function get_promocion_evento($id_evento){

	$query_get = "SELECT * FROM acceso WHERE 
				idevento =  $id_evento 	LIMIT 1";
...
/**/
function get_promocion_proximo_evento($id_evento){

	$query_get = "SELECT * FROM acceso WHERE 
				idevento =  $id_evento 
				AND  			
				DATE(inicio_acceso)  BETWEEN DATE(CURRENT_DATE() )
				...
				if (count($info) == 0 ){
					$info = $this->get_promocion_evento($id_evento);	
				}
				return $info;
}	
/**/
