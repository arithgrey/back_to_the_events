...
/**/
if ( ! function_exists('valida_plural'))
{
  function valida_plural($text , $text2 , $num){
...
if ( ! function_exists('valida_info_completa_resumen'))
{
  function valida_info_completa_resumen($text , $class ,  $extra ){
    ...
    $n_text =  "<p class='list-group-item-text $class '>                         
                ".$text ."
                </p>"; 

    if (trim(strlen($text)) > 450  ) {

      $n_text =  "<p class='list-group-item-text $class ' ".$extra." >                         
                ". substr( $text, 0 , 450 ) ." 
                    <span> 
                      <strong>... más info 
                      </strong>
...
{
  function template_editar( $in_session,  $url ,  $class_extra = '' ){

      if ($in_session ==  1 ){
        //<i class="fa fa-pencil" aria-hidden="true"></i>

        return "<a href='".$url."'>                  
                    <i class='fa fa-pencil ".$class_extra." '>
                    </i>                  
                </a>";  
